...
- Every dealt card must have its animation class (`.dealing` or `.dealing-facedown`) removed in its own `animationend` handler. If those classes are left on after the animation ends, the `forwards` fill keeps the animation active — any later class change that overrides it (e.g. `flash-win`/`flash-lose`) will cause the deal animation to restart from the beginning when removed, making cards visually re-deal themselves. The `onDealComplete` callback must only be invoked for the last card (`i === 3`), but all 4 cards need their class cleaned up.
